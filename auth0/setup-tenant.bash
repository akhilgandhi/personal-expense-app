...
API_NAME=dashboard
API_URL=https://localhost:8443/dashboard
...
then
  ...
  curl -s -H "Authorization: Bearer $AT" -X POST -H "Content-Type: application/json" -d "{\"name\": \"$API_NAME\", \"identifier\": \"$API_URL\", \"scopes\": [{ \"value\": \"account:read\", \"description\": \"Read account information\"}, {\"value\": \"account:write\", \"description\": \"Update account information\"}]}" https://$TENANT/api/v2/resource-servers | jq .
fi
...
then
  ...
  curl -s -H "Authorization: Bearer $AT" -X POST  -H "Content-Type: application/json" -d "{\"client_id\":\"$READER_CLIENT_ID\",\"audience\":\"$API_URL\",\"scope\":[\"account:read\"]}" https://$TENANT/api/v2/client-grants | jq .
  echo
fi
...
then
  ...
  curl -s -H "Authorization: Bearer $AT" -X POST  -H "Content-Type: application/json" -d "{\"client_id\":\"$WRITER_CLIENT_ID\",\"audience\":\"$API_URL\",\"scope\":[\"account:read\",\"account:write\"]}" https://$TENANT/api/v2/client-grants | jq .
  echo
fi
